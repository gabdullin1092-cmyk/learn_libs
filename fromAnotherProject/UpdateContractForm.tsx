...
import { getCurrencyListFn, getVatTypeListFn, getGuaranteeTypeListFn } from "@features/common/functions";
import { getCompanyUsersListFn } from "@features/company/functions";
import { withForm, updateFormOptions, initialSignerValues } from './form-context';

type FileResponse = components['schemas']['FileResponse']
type ContractSignerInput = components['schemas']['ContractSignerInputDto']
...
export const UpdateContractForm = withForm({
    ...updateFormOptions,
    props: {} as UpdateContractFormProps,
    render: function Render({ form, company: receiverCompany, user: receiverUser }) {
        const { t } = useLingui();
        ...
        const [isShowAmountField, setIsShowAmountField] = useState(values.contract.amount != null);
        const [isIndefiniteTerm, setIsIndefiniteTerm] = useState(() => values.contract.endDate == null);

        const getCurrencyList = useServerFn(getCurrencyListFn);
        const getVatTypeList = useServerFn(getVatTypeListFn);
        const getGuaranteeTypeList = useServerFn(getGuaranteeTypeListFn);
        ...
        const { data: guaranteeTypeList } = useQuery({
            queryKey: ['guaranteeTypeList'],
            queryFn: () => getGuaranteeTypeList()
        });
        ...
        const guaranteeTypeCollection = createListCollection({
        ...
        const receiverSigner = values.contract.signers.find((signer: ContractSignerInput) => signer.companyId !== companyId);
            ...
            >
                <Stack gap={6} bg="transparent" borderRadius="24px" w="100%" maxW={900}>
                    <ContractCompanySearch
                        ...
                        onSignerChange={(signer) => {
                            form.setFieldValue('contract.signers', values.contract.signers
                                .filter((item: ContractSignerInput) => item.companyId === companyId)
                                .concat([signer])
                            )
                        }}
                        company={receiverCompany}
                    />
                    <Show when={receiverSigner?.companyId || (receiverSigner?.companyName && receiverSigner?.idn)}>
                    ...
                    <Box bg="white" height="auto" borderRadius="24px" p={{ base: "16px", md: "24px" }}>
                        ...
                        <Box mb={6}>
                            <Text fontSize="16" fontWeight="600" color="gray.800" mb="12px">
                                <Trans>Вид обеспечения</Trans>
                            </Text>

                            <Box display="flex" gap="8px" mb="24px" maxW="100%">
                                <For each={guaranteeTypeCollection.items}>
                                    {(item) => (
                                        <Box
                                            key={item.id}
                                            display="flex"
                                            ...
                                            px="12px"
                                            borderRadius="30px"
                                            bg={(values.contract.sourceId?.toString?.() ?? '') === item.id.toString() ? "bg.contrast" : "bg.lightGray4"}
                                            color={(values.contract.sourceId?.toString?.() ?? '') === item.id.toString() ? "white" : "text.dark"}
                                            cursor="pointer"
                                            whiteSpace="nowrap"
                                            flexShrink={0}
                                            minW="fit-content"
                                            onClick={() => form.setFieldValue('contract.sourceId', String(item.id))}
                                            transition="all 0.2s"
                                            _hover={{
                                                bg: (values.contract.sourceId?.toString?.() ?? '') === item.id.toString() ? "bg.contrast" : "bg.lightGray3"
                                            }}
                                        >
                                ...
                                {(field) => (
                                    <Field.Root required invalid={!!field.state.meta.errors.length}>
                                        ...
                                        <Select.Root
                                            ...
                                            multiple
                                            value={values.contract.signers.filter((signer: ContractSignerInput) => signer.companyId === companyId).map((signer: ContractSignerInput) => signer.userId!)}
                                            onValueChange={(value) => {
                                                field.handleChange([
                                                    ...values.contract.signers.filter((signer: ContractSignerInput) => signer.companyId !== companyId),
